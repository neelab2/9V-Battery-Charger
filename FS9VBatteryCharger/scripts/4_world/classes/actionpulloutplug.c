modded class ActionPullOutPlug : ActionInteractBase
{
	...
	{	
		...
		if (targetObject)
		{
			...
			if (device)
			{
				...
				if (device.IsInherited(FSBatteryCharger))
				{
				device.SetObjectMaterial( 2, FSBatteryCharger.Inv_Materieal );
				FSBatteryCharger charger = FSBatteryCharger.Cast(device);
				charger.iscordedplugged = false;
